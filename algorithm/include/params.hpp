...
struct Params {
  ...
  bool remove_singleton_rows{true};

  /// Eliminate redundant and forcing constraints based on row bounds L_i and
  /// U_i
  bool remove_redundant_forcing{true};

  /// Tighten individual variable bounds using finite constraint limits
  /// (Gondzio's method)
  ...
  static void parse(int argc, char *argv[]) {
    ...
    options.add_options()("instance", "Instance file (positional)",
        ...
        "Remove singleton rows (single non-zero entry)")(
        "remove-redundant-forcing",
        "Remove redundant and forcing constraints based on row bounds")(
        "tighten-bounds",
        "Tighten individual variable bounds using constraint limits")(
        ...
        cxxopts::value<size_t>()->default_value(
            std::to_string(p.refactor_period)))(
        "b,bland_threshold",
        "Multiplier of (m+n) consecutive degenerate pivots before falling back "
        "to Bland's rule (0 = always "
        "Bland)",
        cxxopts::value<double>()->default_value(
            std::to_string(p.bland_threshold)))(
